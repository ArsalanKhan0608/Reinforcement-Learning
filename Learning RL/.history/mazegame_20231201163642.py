...
class ModifiedMazeGameEnv(gym.Env):
    def __init__(self, maze):
        ...
        self.observation_space = spaces.Box(low=-10, high=10, shape=(self.num_rows * self.num_cols * 2,), dtype=np.int32)
    ...
    def reset(self, seed=None, options=None, **kwargs):
        ...
        return self._get_observation_with_position(), {}  # Return observation and an empty info dictionary

    def step(self, action):
        new_pos = np.array(self.current_pos)
        # [Action effects code remains the same]
        ...
        # Penalize for staying in the same position
        if np.array_equal(self.current_pos, new_pos):
            reward -= 0.5

        self.total_reward += reward
        return self._get_observation_with_position(), reward, done, False, {}
    ...
    def _get_observation_with_position(self):
    # Flatten the entire maze into a 1D array and append the agent's position
        state = self.maze.flatten().astype(np.int32)
        pos = np.zeros_like(state)
        pos[self.current_pos[0] * self.num_cols + self.current_pos[1]] = 1
        return np.concatenate([state, pos])
