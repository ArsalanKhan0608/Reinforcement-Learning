...
class ModifiedMazeGameEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if valid_move:
            ...
            # Set the new position to 0 (agent's position)
            # and reset the previous position only if it's not the goal
            if not np.array_equal(prev_pos, self.goal_pos):
                self.maze[prev_pos] = -1  # or self.original_maze[prev_pos] if original_maze is maintained

            if np.array_equal(self.current_pos, self.goal_pos):
                # If it's the goal, indicate the agent has reached the goal
                self.maze[tuple(self.current_pos)] = 0  # Agent is at the goal
            else:
