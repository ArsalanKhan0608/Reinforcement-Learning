...
class ModifiedMazeGameEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if valid_move:
            # Reset the previous position to its original value (assuming -1 for empty space)
            self.maze[tuple(self.current_pos)] = -1

            # Update the current position to the new position
            self.current_pos = new_pos

            # If it's the goal, keep the goal value. Otherwise, set the position to 0 (agent's position)
